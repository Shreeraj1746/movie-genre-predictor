...
def test_health_endpoint_no_model(test_client):
    ...
    with patch("src.api.main.model", None), patch("src.api.main.feature_extractor", None), \
...
def test_health_endpoint_with_model(test_client, mock_model, mock_feature_extractor):
    ...
    with patch("src.api.main.model", mock_model), \
         patch("src.api.main.feature_extractor", mock_feature_extractor):
        response = test_client.get("/health")
        assert response.status_code == 200
...
def test_model_info_endpoint(test_client, mock_model, mock_feature_extractor):
    ...
    with patch("src.api.main.model", mock_model), \
         patch("src.api.main.feature_extractor", mock_feature_extractor), \
         patch("src.api.main.get_model_path", return_value="/path/to/logreg_20230101_123456/model.joblib"):
