...
def load_metadata(filepath: Path) -> pd.DataFrame:
    ...
    columns = [
        "movie_id", "movie_name", "movie_release_date", "movie_box_office",
    ...
    df["release_year"] = pd.to_numeric(
        df["movie_release_date"].str.extract(r"(\d{4})")[0],
        errors="coerce"
    )
...
def load_plot_summaries(filepath: Path) -> pd.DataFrame:
    ...
    df = pd.read_csv(
        filepath, sep="\t", header=None, names=["movie_id", "plot"]
    )
...
def merge_and_clean_data(
    ...
    # Merge on movie_id
    merged_df = pd.merge(
        metadata_df, plots_df, on="movie_id", how="inner"
    )
    ...
    # Keep only common genres (minimum 100 examples)
    genre_counts = merged_df["primary_genre"].value_counts()
    common_genres = genre_counts[genre_counts >= 100].index.tolist()
    merged_df = merged_df[merged_df["primary_genre"].isin(common_genres)]

    # Select final columns for the processed dataset
    final_df = merged_df[[
        "movie_id", "movie_name", "primary_genre",
        "plot_clean", "release_year", "movie_runtime"
    ]].rename(columns={
        "movie_name": "title",
        "primary_genre": "genre",
        "plot_clean": "plot",
...
def create_data_splits(
    ...
    train_df, val_df = train_test_split(
        ...
        random_state=random_state, stratify=train_val_df["genre"]
    )

    logger.info(f"Created splits: train={len(train_df)}, "
...
def save_processed_data(
    ...
    full_df: pd.DataFrame, output_dir: Path
) -> None:
    """
    ...
    genre_labels = {
        idx: genre for idx, genre in enumerate(sorted(full_df["genre"].unique()))
    }
    pd.DataFrame({
        "index": list(genre_labels.keys()),
        "genre": list(genre_labels.values())
    }).to_csv(output_dir / "genre_labels.csv", index=False)

    logger.info(f"Saved processed datasets to {output_dir}")


def preprocess_data(output_dir: Optional[Path] = None) -> bool:
    """
